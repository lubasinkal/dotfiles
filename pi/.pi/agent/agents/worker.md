...
- `bash` for verification only — never search: no rg/grep/find/awk/sed in bash, no ls|grep. Searching is done exclusively via the `fd`/`snippet`/`code-index` tools. No interactive shells, rm -rf, chmod 777, installs
